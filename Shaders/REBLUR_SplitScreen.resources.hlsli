...
NRD_INPUTS_START
    NRD_INPUT( Texture2D<float>, gIn_ViewZ, t, 0 )
        ...
        #if( NRD_MODE == SH )
            NRD_INPUT( Texture2D<REBLUR_SH_TYPE>, gIn_DiffSh, t, 3 )
            NRD_INPUT( Texture2D<REBLUR_SH_TYPE>, gIn_SpecSh, t, 4 )
        #endif
    #elif( NRD_DIFF )
        NRD_INPUT( Texture2D<float4>, gIn_Diff, t, 1 )
        #if( NRD_MODE == SH )
            NRD_INPUT( Texture2D<REBLUR_SH_TYPE>, gIn_DiffSh, t, 2 )
        #endif
    #else
        NRD_INPUT( Texture2D<float4>, gIn_Spec, t, 1 )
        #if( NRD_MODE == SH )
            NRD_INPUT( Texture2D<REBLUR_SH_TYPE>, gIn_SpecSh, t, 2 )
        #endif
    #endif
NRD_INPUTS_END
...
NRD_OUTPUTS_START
        ...
        #if( NRD_MODE == SH )
            NRD_OUTPUT( RWTexture2D<REBLUR_SH_TYPE>, gOut_SpecSh, u, 1 )
        #endif
    #endif
NRD_OUTPUTS_END
